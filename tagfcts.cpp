...
{
	//std::cout << "[" << albumName << "] (" << artistName << ") -> " << nTracks << " : " << date << std::endl;
...
bool TagInfos::compareSongData(sqlite3* db)
{
	...
	WHERE songs.path=? AND songs.id_dirName=directories.id AND songs.id_album=albums.id", -1, &requestStatement, 0), 0);
